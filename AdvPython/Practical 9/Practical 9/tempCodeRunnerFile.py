...
input_file = r"D:\STUDY\SEM 5\Python Lab\Practical 9\Practical 9\expenses2.csv"
backup_file = r"D:\STUDY\SEM 5\Python Lab\Practical 9\Practical 9\expenses_backup.csv"
...
for index, row in df.iterrows():
    if pd.isna(row['Category']) or row['Category'] == "":  # Check for NaN or empty string
        ...
        df.at[index, 'Category'] = category.strip()  # Update the DataFrame with the entered category
...
for category in df['Category'].unique():
    if pd.notna(category):  # Only ask for non-null categories
...
for category in monthly_budget:
    if category in remaining_monthly_budget.columns:  # Check if category exists in expenses
