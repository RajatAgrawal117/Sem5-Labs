...
def load_train_data(file_path):
    ...
    data["Available Seats"] = pd.to_numeric(data["Available Seats"], errors='coerce')
...
def load_passenger_data(file_path):
    ...
    data = pd.read_csv(file_path)
    # Convert columns to appropriate data types
    data["Number of tickets"] = pd.to_numeric(data["Number of tickets"], errors='coerce')
    return data
...
def update_seat_availability(train_data, train_id, num_tickets):
    train_data.loc[train_data["Train ID"] == train_id, "Available Seats"] -= num_tickets

# Generate report showing train details
def generate_train_report(train_data):
    report = train_data[["Train Name", "Source Station", "Destination Station", "Available Seats"]]
...
def generate_revenue_report(train_data, passenger_data):
    revenue = {}
    for _, row in passenger_data.iterrows():
        train_id = row["Train ID"]
        num_tickets = row["Number of tickets"]
        fare = train_data.loc[train_data["Train ID"] == train_id, "Total Fare"].values[0]
    ...
    revenue_df = pd.DataFrame(list(revenue.items()), columns=["Train ID", "Total Revenue"])
    ...
    try:
        ...
        for _, row in passenger_data.iterrows():
            ...
            if check_seat_availability(train_data, train_id, num_tickets):
                ...
                print(f"Insufficient seats for {passenger_name} on train {train_id}.")

        generate_train_report(train_data)
        generate_revenue_report(train_data, passenger_data)
