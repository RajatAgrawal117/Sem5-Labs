...
def validate_entry(entry):
    required_fields = ['country', 'date', 'confirmed_cases', 'deaths', 'recovered']
...
folder_path = ''
file_names = ['Germany.json', 'Russia.json', 'Ukraine.json', 'Italy.json', 'USA.json', 'Japan.json', 'Europe.json', 'England.json']
combined_data = []
        ...
        with open(file_path, 'r') as file:
            ...
            for entry in data:
                if validate_entry(entry):
                    ...
                    print(f"Error: Missing or invalid data in file {file_name} entry: {entry}")
...
filtered_data = [item for item in combined_data if item['country'] == selected_country]
...
with open(output_file_path, 'w') as output_file:
    if filtered_data:
        ...
        for record in filtered_data:
            ...
            output_file.write(f"Confirmed Cases: {record['confirmed_cases']['total']} (New: {record['confirmed_cases']['new']})\n")
            output_file.write(f"Deaths: {record['deaths']['total']} (New: {record['deaths']['new']})\n")
            output_file.write(f"Recovered: {record['recovered']['total']} (New: {record['recovered']['new']})\n")
...
for item in combined_data:
    ...
    if country not in country_case_totals:
        ...
        country_case_totals[country] = max(country_case_totals[country], total_cases)

sorted_countries = sorted(country_case_totals.items(), key=lambda x: x[1], reverse=True)
